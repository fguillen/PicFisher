...
class TestPicFisher < Minitest::Test
  ...
  def test_fish
    ...
    images = [
      ...
      { url: "https://example.com/other/Se%20parated.jpg",
        output_file_name: "https_example_com_other_Se_20parated.jpg" },
    ...
    Dir.mktmpdir do |output_directory_path| # Not used, but it should exist to overpass the validations
      images.each do |image|
        PicFisher::Downloader.expects(:download).with(image[:url],
                                                      "#{output_directory_path}/#{image[:output_file_name]}")
      end
  ...
  def test_fish_integration_mode
    ...
    Dir.mktmpdir do |output_directory_path|
      PicFisher.fish(images_file_path, output_directory_path)
      images.each do |image|
        hash_original = Digest::MD5.file(fixture(image[:fixture]))
        hash_downloaded = Digest::MD5.file("#{output_directory_path}/#{image[:output_file_name]}")
        assert(File.exist?("#{output_directory_path}/#{image[:output_file_name]}"), "Excepted to find file '#{image[:output_file_name]}'")
        assert_equal(hash_original, hash_downloaded, "Excepted to same has for file '#{image[:url]}'")
      end
    end
  end
end
